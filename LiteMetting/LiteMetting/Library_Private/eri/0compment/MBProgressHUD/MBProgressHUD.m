...
//----------- eri -----------
BOOL isInLockState;// 提示框 不消失也不出现

-(void)ee_setInLockState:(BOOL)isinlock{
    isInLockState = isinlock;
}
...
+ (MBProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    ...
    //洪湃添加的代码 --  正确性 待考证
	for (MBProgressHUD *v in [view subviews]) {
...
-(void)ee_insertCloseBtn:(id)requestDelegate{
    _requestDelegate = requestDelegate;
    
    closeBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    //[closeBtn setFrame:CGRectMake(indicator.frame.origin.x+indicator.frame.size.width+4, indicator.frame.origin.y-indicator.frame.size.height*0.5-10, 30 ,30 )];
    [closeBtn setFrame:CGRectMake(542,326, 30 ,30 )];
    
    
    [closeBtn setTitle:@"X" forState:UIControlStateNormal];
    [closeBtn.titleLabel setFont:[UIFont systemFontOfSize:18]];
    [closeBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [closeBtn setTitleColor:[UIColor orangeColor] forState:UIControlStateSelected];
    [closeBtn setTitleColor:[UIColor orangeColor] forState:UIControlStateHighlighted];
...
-(void)ee_removeCustomCloseButton{
    if (closeBtn!=nil) {
...
-(void)clickCloseButton{
    trace(@"fasdf");
    trace(@"%@",_requestDelegate);
    [ _requestDelegate performSelector:@selector(eeCancelRequestFromHUDs)];
